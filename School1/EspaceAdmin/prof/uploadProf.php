  ...
  ;
       ...
       // $CIN=str_replace(' ', '', $_POST['CIN']);
        ...
        for ($j = 0; $j <count($nom); $j++)
         ...
         $insert = $db->query("INSERT into professeur(image,nom,prenom,adresse,codePostal,mail,villeN,anneeS,dateN,sexe,tel,id_admin,mdp,CIN) VALUES ('$imgContent','$nom[$j]','$prenom[$j]','$adresse[$j]','$codeP[$j]','$email[$j]','$villeN[$j]','$anneeS[$j]','$dateN[$j]','$sexe[$j]','$tel[$j]','$id_admin','$mdp[$j]','$CIN[$j]')");
        if($insert){
                $status = 'success';
