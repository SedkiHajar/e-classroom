...
<?php   $result = $db->query("SELECT * FROM matiere m join classe c join matclass t
 where c.anneeS='$an' and m.id=t.id_Mat and t.id_Class=c.id ");
     $nbrMat=0;
      ?>
     <?php while($row = $result->fetch_assoc()){
      $nbrMat++;}
        ...
        <div class="container-fluid">
          ...
          <!-- formulaire des etudiant a ajouter  -->
           ...
           <form action="uploadCl.php" role="form" method="post" enctype="multipart/form-data">
              ...
              <label for="nom">Nom Du Prof  <?php echo $_SESSION['mail'];?> </label>
              <select class="custom-select" name="CIN[]" id="">
                <option selected value="-1">sélectionner le prof</option>
                <?php
                //echo 'prof:'.$_SESSION['mail'];
              //définir la requete
               $result = $db->query("SELECT * FROM professeur where anneeS=$an");?>
